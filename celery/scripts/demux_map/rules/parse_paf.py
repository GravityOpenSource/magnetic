...
def parse_line(line, header_dict):
    ...
    if values["read_name"] in header_dict:
        values["barcode"], values["start_time"] = header_dict[
                ...
                if len(reference_options[k]) == 1:
                    ...
                    for i in reference_options[k]:
                        if len(i) == 3:
                            ...
                            overlap, length = check_overlap((opt_start, opt_end),
                                                            (int(mapping["coord_start"]), int(mapping["coord_end"])))
                            if overlap:
...
if __name__ == '__main__':
    ...
    with open(str(args.report), "w") as csv_report:
        ...
        csv_report.write(
            f"read_name,read_len,start_time,barcode,best_reference,ref_len,start_coords,end_coords,num_matches,mapping_len{ref_option_header}\n")
